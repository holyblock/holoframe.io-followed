...
const readAnimationSequence = (obj: any) => {
  const animationSequence: Array<AnimationSequence> = [];
  for (const d of obj) {
...
export class AnimationGenerator {
  ...
  public prediction = () => {
    ...
    const elapsed =
      ((currTime - this._startTime) / 1000) % this._animationDuration;

    const blendshapes = {};

    blendshapes['eyeBlink_L'] = findAnimationValue(
      this._leftBlinkSeq,
      elapsed,
      this._renderInterval
    );
    blendshapes['eyeBlink_R'] = findAnimationValue(
      this._rightBlinkSeq,
      elapsed,
      this._renderInterval
    );
    blendshapes['jawOpen'] = findAnimationValue(
      this._mouthOpenSeq,
      elapsed,
      this._renderInterval
    );
    blendshapes['mouthSmile_L'] = findAnimationValue(
      this._smileSeq,
      elapsed,
      this._renderInterval
    );
    blendshapes['mouthSmile_R'] = findAnimationValue(
      this._smileSeq,
      elapsed,
      this._renderInterval
    );
    blendshapes['eyeSquint_L'] = findAnimationValue(
      this._smileSeq,
      elapsed,
      this._renderInterval
    );
    blendshapes['eyeSquint_R'] = findAnimationValue(
      this._smileSeq,
      elapsed,
      this._renderInterval
    );

    const lookX = findAnimationValue(
      this._lookXSeq,
      elapsed,
      this._renderInterval
    );
    const lookY = findAnimationValue(
      this._lookYSeq,
      elapsed,
      this._renderInterval
    );
    ...
    // apply expressions
    const peace = findAnimationValue(
      this._peaceSeq,
      elapsed,
      this._renderInterval
    );
    const fieryEyes = findAnimationValue(
      this._fieryEyesSeq,
      elapsed,
      this._renderInterval
    );
    if (this._live2dModel) {
      if (peace != 0 || this._prevPeaceValue != 0) {
        const updated = this.updateExpression(
          peace,
          this._prevPeaceValue,
          'Peace'
        );
        if (updated) {
          this._live2dModel.activateExpressions([
            ...this._activatedExpressions,
          ]);
        }
      }
