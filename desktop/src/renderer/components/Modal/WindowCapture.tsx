...
const WindowCapture = (props: WindowCaptureProps) => {
  ...
  useEffect(() => {
    (async () => {
      if (isOpen) {
        ...
        ipcRenderer.on('window-sources', async (evt, newSources) => {
  ...
  const updatePreviewVideo = async (sourceID: string) => {
    try {
      ...
      const video = document.getElementById('window') as HTMLVideoElement;
