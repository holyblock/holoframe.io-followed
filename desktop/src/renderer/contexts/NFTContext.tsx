...
import { AvatarModel, Placement, NFTMetadata } from 'renderer/types';
import { useAuth } from 'renderer/contexts/AuthContext';
import { aApiKey } from 'renderer/settings';
import { NFTCollection, SceneType } from 'renderer/types/types';
import { Scene } from 'renderer/utils/scene';
import { db } from 'renderer/utils/clients/dbClient';
...
export const NFTProvider = ({ children }: any) => {
  ...
  const { network, userAddress, isAuthenticated } = useAuth();
  ...
  // Fetch NFTs upon authentication or network switch
  useEffect(() => {
    (async () => {
      if (userAddress && network && isAuthenticated) {
  ...
  }, [userAddress, network, isAuthenticated]);
  ...
  const parseNFTs = async (allNFTs) => {
    for (const nft of allNFTs) {
      ...
      if (Object.keys(partnerProjectsConfig).includes(symbol)) {
        ...
        // Get model url and populate information
        if (verified) {
      ...
      if (!supportedCommunities.current.has(symbol)) {
        const currCommunityCollection = getCommunityCollection(network, nft);
  ...
  // Clear NFT cache in preparation for account or network switch
  const reset = async () => {
