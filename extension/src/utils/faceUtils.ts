...
let callerVideoUnflipped = false;
let flipVideoCSS: Array<any> = [];
...
function getAllCSSRules() {
  let rules: any = {};
  // only keep stylesheets that are loaded from the same domain or where href is null
  // source: https://betterprogramming.pub/how-to-fix-the-failed-to-read-the-cssrules-property-from-cssstylesheet-error-431d84e4a139
  ...
  for (let style of styleSheets) {
    if (style instanceof CSSStyleSheet && style.cssRules) {
      let cssRules: any = style.cssRules;
...
export function checkUnflipped() {
  return callerVideoUnflipped;
}

export function flipBlendShapes(inputBlendShapes: any) {
  var blendShapesDict: any = {};
...
export function unflipCallerVideo() {
  if (!callerVideoUnflipped) {
    const cssRules = getAllCSSRules();
    flipVideoCSS = [];
    let allVideos = document.getElementsByTagName("video");
    for (let video of allVideos) {
      if (video.id != "input-video") {
        for (const key in cssRules) {
          if (video.matches(key)) {
            if (
              cssRules[key].style.transform == "scaleX(-1)" ||
              cssRules[key].style.transform == "scale(-1, 1)"
            ) {
              flipVideoCSS.push(cssRules[key]);
              cssRules[key].style.transform = ""; // unflip
              callerVideoUnflipped = true;
            }
          }
        }
      }
...
export function reflipCallerVideo() {
  if (callerVideoUnflipped) {
    for (let css of flipVideoCSS) {
      css.style.transform = "scaleX(-1)"; // reflip
    }
    callerVideoUnflipped = false;
  }
}
