...
export class BoneCache {
  ...
  public addBone = (node: THREE.Bone) => {
  ...
  public rotateNode = (
    ...
    if (name in this.bones) {
      this.bones[name].rotation.x = rotateX;
      this.bones[name].rotation.y = rotateY;
      this.bones[name].rotation.z = rotateZ;
      return true;
    } else {
      return false;
    }
  ...
  public additiveRotateNode = (
    ...
    if (name in this.bones) {
      this.bones[name].rotation.x += x;
      this.bones[name].rotation.y += y;
      this.bones[name].rotation.z += z;
      return true;
    } else {
      return false;
    }
...
export class BreathingMotor {
  ...
  public getSinOffset = (freq: number, updateT: boolean) => {
    const t = Date.now();
    const angle = Math.sin(t / freq);
    const prevAngle = Math.sin(this.prevT / freq);
    if (updateT) {
      this.prevT = t;
    }
...
export const appleARKitToAlter = {
