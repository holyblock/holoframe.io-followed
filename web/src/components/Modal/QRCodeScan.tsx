...
const ScanResult = ({ isDesktopAuth, session, onSuccess, onRegenerate }) => {
  ...
  useEffect(() => {
    if (status === "authenticated") {
      setTimeout(async () => {
        if (!isDesktopAuth) {
          await localforage.setItem("tokenproofSession", session);
        }
        onSuccess(session);
      }, 1000);
    }
...
const QRCodeScan = ({ isDesktopAuth = false, onSuccess }) => {
  const { tokenproofSession, setTokenproofSession } = useAuth();
  ...
  const handleGenerate = useCallback(() => {
    ...
    fetch(`${config.apiUrl}/auth/tokenproof`, {
      method: "post",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ nonce }),
    })
      .then((res) => res.json())
      .then((json) => setImgData(json.qrcode_image));
  }, [sendMessage]);
