...
class ProjectFinishTime(object):
  ...
  def update_progress(self, step):
    ...
    ud_hr = int(math.floor(undone_time / 3600))
    ud_min = int(math.floor((undone_time - ud_hr * 3600) / 60))
    ud_sec = undone_time % 60

    # estimated finish time
    finish_time = curr_time + undone_time
    finish_time = datetime.fromtimestamp(finish_time).strftime(
        "%Y-%m-%d %I:%M:%S")

    if self.print_info:
      print('{} out of {} steps, '.format(step, self.total_steps) +
            'time left: {}h {}m {}s, '.format(ud_hr, ud_min, round(ud_sec, 2)) +
            'eta: {}'.format(finish_time),
            end=self.end)
